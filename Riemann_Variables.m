...
R_L=0.0019;         % Radius of the curvature  and x=L
...
%% Initializing the HLL Flux vectors for U_1 
 ...
 %FU_11_half_plus=zeros(m-1,n+1);
 %FU_21_half_plus=zeros(m-1,n+1);

 % FU_11_half_minus=zeros(m-1,n);
 % FU_21_half_minus=zeros(m-1,n);

%% Initializing the HLL Flux vectors for U_2
 % FU_12=zeros(m-1,n+1);
 % FU_22=zeros(m-1,n+1);

%% Initialize Q at x=0 and x=L for U_1
%Qbound0=f_0*ones(m,1); %zeros(m,1);%f_0*ones(m,1);
...
%% Caluclate Abound0 for U_1
  %polynomial_01=[gamma^4 -(unear01 - 4*cnear01)^4 4*Qbound0(j-1)*((unear01 - 4*cnear01)^3) -6*Qbound0(j-1)^2*((unear01 - 4*cnear01)^2) 4*Qbound0(j-1)^3*(unear01 - 4*cnear01) -Qbound0(j-1)^4];
 ...
 % Declare coefficients of the polynomial bar_d1*A^(5/2)=bar_a1A^2 + bar_b1*A +bar_c1, for the BC a x=L
  ...
  %dQdx(j-1,1)=FU_11(j-1,1)/dx;

  dAdx(j-1,1)=(U_1(j-1,1)-Ghost_A1_0(j-1))/dx;
  % FU_11(j-1,1)=Qbound0(j-1);
  % FU_21(j-1,1)=Flux(Qbound0(j-1), Abound01(j-1), rho, beta1, A_0); 
  %% Using the Area of first element for the boundary interface flux calculation
  %FU_21(j-1,1)=Flux(Qbound0(j-1), U_1(j-1,2), rho, beta1, A_0); 
  ...
  %% First we define slope limiters (van Albada)
  %Phi_A1(j-1,1)=(r_A1(j-1,1)^2+r_A1(j-1,1))/(1+r_A1(j-1,1)^2);%
  %Phi_Q1(j-1,1)=(r_Q1(j-1,1)^2+r_Q1(j-1,1))/(1+r_Q1(j-1,1)^2);%
  ...
  %u_half_plus(j-1,1)=u_1(j-1,1) + (dx/2)*Du(1)*(A_half_minus(j-1,1)/U_1(j-1,1));
  %u_half_minus(j-1,1)=u_1(j-1,1) - (dx/2)*Du(1)*(A_half_plus(j-1,1)/U_1(j-1,1));
  
  %Q_half_minus(j-1,1)=U_1(j-1,n+1) + (dx/2)*DQ(1);%A_half_minus(j-1,1)*u_half_minus(j-1,1);
  %Q_half_plus(j-1,1)=U_1(j-1,n+1) - (dx/2)*DQ(1);%A_half_plus(j-1,1)*u_half_plus(j-1,1);
  ...
  %if c_11G0(j-1)>=0
   %FU_11(j-1,1)=Qbound0(j-1);
   %FU_21(j-1,1)=Flux(Qbound0(j-1),Abound01(j-1),rho, beta1, A_0);
  %FU_11(j-1,1)=Qbound0(j-1);%Q1_half_minus(j-1,1);%Qbound0(j-1);%Q1_half_minus(j-1,1);%Qbound0(j-1);%Ghost_Q1_0(j-1);
  %FU_21(j-1,1)=Flux(Qbound0(j-1),Abound01(j-1),rho, beta1, A_0);%Flux(Q1_half_minus(j-1,1),A1_half_minus(j-1,1),rho, beta1, A_0);%Flux(Qbound0(j-1),Abound01(j-1),rho, beta1, A_0);%Flux(Ghost_Q1_0(j-1), Ghost_A1_0(j-1), rho, beta1, A_0);
  %FU_21(j-1,1)=Flux(Qbound0(j-1),Abound01(j-1),rho, beta1, A_0);%Flux(Qbound0(j-1),A1_half_minus(j-1,1),rho, beta1, A_0);%Flux(Qbound0(j-1),Abound01(j-1),rho, beta1, A_0);%Flux(Ghost_Q1_0(j-1), Ghost_A1_0(j-1), rho, beta1, A_0);
  
  %elseif c_11G0(j-1)<0 && c_21G0(j-1)>0
  ...
  %elseif c_21G0(j-1)<=0
   ...
   %FU_11(j-1,1)=U_1(j-1,n+1);
   %FU_21(j-1,1)=Flux(U_1(j-1,n+1), U_1(j-1,1), rho, beta1, A_0);
  %end 

  % FU_11_half_plus(j-1,1)= FU_11(j-1,1) + A_half_plus(j-1,1);
  % FU_21_half_plus(j-1,1)= FU_21(j-1,1) + Q_half_plus(j-1,1);

  %FU_11_half_minus(j-1,1)= FU_11(j-1,1) + A_half_minus(j-1,1);
  %FU_21_half_minus(j-1,1)= FU_21(j-1,1) + Q_half_minus(j-1,1);
  %% Flux at x=L
  ...
  % FU_11(j-1,n+1)=QboundL1(j-1);
  % FU_21(j-1,n+1)=Flux(QboundL1(j-1), AboundL1(j-1), rho, beta1, A_0); 
  
  %% Second-order reconstruction without minmod 
  %% First we define slope limiters (van Leer)
   r_A1(j-1,n)=(U_1(j-1,n)-U_1(j-1,n-1))/(Ghost_A1_L(j-1)-U_1(j-1,n));
   %r_Q1(j-1,n)=(U_1(j-1,n+1)-Ghost_Q1_0(j-1))/(U_1(j-1,n+2)-U_1(j-1,n+1));
   Phi_A1(j-1,n)=(r_A1(j-1,n)+abs(r_A1(j-1,n)))/(1+abs(r_A1(j-1,n)));
   Phi_Q1(j-1,n)=2;
   
   %% First we define slope limiters (van Albada)
   %Phi_A1(j-1,n)=(r_A1(j-1,n)^2+r_A1(j-1,n))/(1+r_A1(j-1,n)^2);%
   %Phi_Q1(j-1,n)=1;%2; %Because r_Q1=infinity for the last cell for %(r_Q1(j-1,1)+abs(r_Q1(j-1,1)))/(1+abs(r_Q1(j-1,1)));
  ...
  %% Flux for the n+1th interface (i.e., n+1/2) based on Ghost cell and last cell (cell n)
  %c_11GL(j-1)= min([lambda1(Ghost_Q1_L(j-1),Ghost_A1_L(j-1),rho,beta1,A_0), lambda1(U_1(j-1,2*n),U_1(j-1,n),rho,beta1,A_0), lambda2(Ghost_Q1_L(j-1),Ghost_A1_L(j-1),rho,beta1,A_0), lambda2(U_1(j-1,2*n),U_1(j-1,n),rho,beta1,A_0)]);
  %c_21GL(j-1)= max([lambda1(Ghost_Q1_L(j-1),Ghost_A1_L(j-1),rho,beta1,A_0), lambda1(U_1(j-1,2*n),U_1(j-1,n),rho,beta1,A_0), lambda2(Ghost_Q1_L(j-1),Ghost_A1_L(j-1),rho,beta1,A_0), lambda2(U_1(j-1,2*n),U_1(j-1,n),rho,beta1,A_0)]);
  %c_11GL(j-1)= min([lambda1(Q1_half_plus(j-1,n), A1_half_plus(j-1,n),rho,beta1,A_0), lambda1(Q1_half_minus(j-1,i),A1_half_minus(j-1,i),rho,beta1,A_0), lambda2(Q1_half_plus(j-1,i-1), A1_half_plus(j-1,i-1),rho,beta1,A_0), lambda2(Q1_half_minus(j-1,i),A1_half_minus(j-1,i),rho,beta1,A_0)]);
  %c_21GL(j-1)= max([lambda1(Q1_half_plus(j-1,n), A1_half_plus(j-1,n),rho,beta1,A_0), lambda1(Q1_half_minus(j-1,i),A1_half_minus(j-1,i),rho,beta1,A_0), lambda2(Q1_half_plus(j-1,i-1), A1_half_plus(j-1,i-1),rho,beta1,A_0), lambda2(Q1_half_minus(j-1,i),A1_half_minus(j-1,i),rho,beta1,A_0)]);

  %if c_11GL(j-1)>=0
  QboundL1(j-1)=U_1(j-1,2*n);
  % FU_11(j-1,n+1)=QboundL1(j-1);
  % FU_21(j-1,n+1)=Flux(QboundL1(j-1),AboundL1(j-1),rho, beta1, A_0);
  %FU_11(j-1,n+1)=QboundL1(j-1);%Q1_half_plus(j-1,n);%Qbound0(j-1);%Ghost_Q1_0(j-1);
  %FU_21(j-1,n+1)=Flux(QboundL1(j-1),AboundL1(j-1),rho, beta1, A_0);%Flux(Q1_half_plus(j-1,n),A1_half_plus(j-1,n),rho, beta1, A_0);%Flux(Qbound0(j-1),Abound01(j-1),rho, beta1, A_0);%Flux(Ghost_Q1_0(j-1), Ghost_A1_0(j-1), rho, beta1, A_0);
  %FU_11(j-1,n+1)=Q1_half_plus(j-1,n);%Qbound0(j-1);%Ghost_Q1_0(j-1);
  %FU_21(j-1,n+1)=Flux(Q1_half_plus(j-1,n),A1_half_plus(j-1,n),rho, beta1, A_0);%Flux(Qbound0(j-1),Abound01(j-1),rho, beta1, A_0);%Flux(Ghost_Q1_0(j-1), Ghost_A1_0(j-1), rho, beta1, A_0);
  %FU_11(j-1,n+1)= U_1(j-1,2*n);
  %FU_21(j-1,n+1)=Flux(U_1(j-1,2*n), U_1(j-1,n), rho, beta1, A_0);
  %elseif c_11GL(j-1)<0 && c_21GL(j-1)>0
  ...
  %FU_11(j-1,n+1)=Ghost_Q1_L(j-1);
  %FU_21(j-1,n+1)=Flux(Ghost_Q1_L(j-1), Ghost_A1_L(j-1), rho, beta1, A_0);
  %end
  %% Calculating the derivatives of [A Q]
   %% $dQ/dx$ at x=0
  %dQdx(j-1,1)=(Q1_half_plus(j-1,2)-Qbound0(j-1))/(2*dx);
  %% $dA/dx$ at x=0
  %dAdx(j-1,1)=(A1_half_plus(j-1,1)-Abound01(j-1))/(dx);
  %% $dQ/dx$ at x=L
  %dQdx(j-1,n)=(Q1_half_plus(j-1,n)-Q1_half_minus(j-1,n))/(dx);
  %% $dA/dx$ at x=L
  %dAdx(j-1,n)=(A1_half_plus(j-1,n)-A1_half_minus(j-1,n))/(dx);
...
%% Define the values c1 and c2 for U_1
   ...
   if i<n
    ...
    %% First we define slope limiters (van Albada)
    %Phi_A1(j-1,i)=(r_A1(j-1,i)^2+r_A1(j-1,i))/(1+r_A1(j-1,i)^2);%
    %Phi_Q1(j-1,i)=(r_Q1(j-1,i)^2+r_Q1(j-1,i))/(1+r_Q1(j-1,i)^2);%
...
% %% Calculating the derivatives of [A Q]
%    %% $dQ/dx$ at x=0
%   dQdx(m,1)=(Q1_half_plus(m,2)-Qbound0(m))/(2*dx);
%   %% $dA/dx$ at x=0
%   dAdx(m,1)=(A1_half_plus(m,1)-Abound01(m))/(dx);
%   %% $dQ/dx$ at x=L
%   dQdx(m,n)=(Q1_half_plus(m,n)-Q1_half_minus(m,n))/(dx);
%   %% $dA/dx$ at x=L
%   dAdx(m,n)=(A1_half_plus(m,n)-A1_half_minus(m,n))/(dx);
...
figure(3)
plot(x1,U_1(121863,n+1:2*n),'LineWidth',2);
% figure(3)
% plot(T1, U_1(1:m-50,n+1), T1, U_1(1:m-50,n+12), T1, U_1(1:m-50,n+n/2),T1, U_1(1:m-50,n+37), T1, U_1(1:m-50,2*n), T1, Qbound0(1:m-50),'--', 'LineWidth',2); 
% xlim([0 3]);
% fontsize(50,"points")
% legend('$Q_1(0^+,t)$','$Q_1(L/4,t)$','$Q_1(L/2,t)$','$Q_1(3L/4,t)$','$Q_1(L,t)$','$Q_{in}(t)$','Interpreter','latex')
% xlabel('$t [sec]$','Interpreter','latex');
...
% figure(4)
% plot(x1, dQdx(m-50,1:n));%, T1, dQdx(m-50,12), T1, dQdx(1:m-50,n/2),T1, dQdx(1:m-50,37), T1, dQdx(1:m-50,n), T1, Qbound0(1:m-50),'--', 'LineWidth',2); 
% xlim([0 3]);
% fontsize(50,"points")
% legend('$Q_1(0^+,t)$','$Q_1(L/4,t)$','$Q_1(L/2,t)$','$Q_1(3L/4,t)$','$Q_1(L,t)$','$Q_{in}(t)$','Interpreter','latex')
% xlabel('$t [sec]$','Interpreter','latex');
...
% grid on
% figure(1)
% plot(T, U_1(1:m,n+1), T, U_1(1:m,n+12), T, U_1(1:m,n+n/2),T, U_1(1:m,n+37), T, U_1(1:m,2*n), T, Qbound0(1:m),'--', 'LineWidth',2); 
% xlim([0 3]);
% fontsize(50,"points")
% legend('$Q_1(0^+,t)$','$Q_1(L/4,t)$','$Q_1(L/2,t)$','$Q_1(3L/4,t)$','$Q_1(L,t)$','$Q_{in}(t)$','Interpreter','latex')
% xlabel('$t [sec]$','Interpreter','latex');
...
% %% Plot Area A over time t
% figure(2)
% plot(T, U_1(1:m,1), T,  U_1(1:m,12), T,  U_1(1:m,n/2), T,  U_1(1:m,37), T, U_1(1:m,n), T, Abound01(1:m), '--','LineWidth',2); % T, Abound02, '--y', T, Abound01, 'r','LineWidth',2); %x, 0.0000014 + 3.45*0.0000001*sin(w*x),'b');
% xlim([0 3]);
% fontsize(50,"points")
% legend('$A_1(0^+,t)$','$A_1(L/4,t)$','$A_1(L/2,t)$','$A_1(3L/4,t)$', '$A_1(L,t)$', '$A_1(0,t)$','Interpreter','latex')
% xlabel('$t [sec]$','Interpreter','latex');
...
% figure(3)
% plot(T1, U_1(1:m-50,2*n), T1, Qbound0(1:m-50),'--', 'LineWidth',2); 
% xlim([0 3]);
% legend('$Q_1(L,t)$','$Q_{in}(t)$','Interpreter','latex')
% xlabel('$t [sec]$','Interpreter','latex');
% ylabel('$Q [m^3~sec^{-1}]$','Interpreter','latex');
% grid on